...
func (r *PostgresStudentRepository) Create(ctx context.Context, s *student.Student) error {
	query := `
        INSERT INTO students (id, tenant_id, user_id, first_name, last_name, birth_date, gender, created_at, updated_at)
...
func (r *PostgresStudentRepository) GetByID(ctx context.Context, id uuid.UUID) (*student.Student, error) {
	query := `
        SELECT id, tenant_id, user_id, first_name, last_name, birth_date, gender, created_at, updated_at
...
func (r *PostgresStudentRepository) Update(ctx context.Context, s *student.Student) error {
	query := `
        UPDATE students SET user_id = $1, first_name = $2, last_name = $3, birth_date = $4,
            gender = $5, updated_at = $6
        WHERE id = $7
    `
	s.UpdatedAt = time.Now()
	_, err := r.db.Exec(ctx, query,
		s.UserID, s.FirstName, s.LastName, s.BirthDate, s.Gender, s.UpdatedAt, s.ID,
	)
	return err
}
