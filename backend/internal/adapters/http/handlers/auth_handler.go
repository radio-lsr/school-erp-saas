...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Tentative de connexion: email=%s", req.Email)

	user, err := h.app.UserRepo.GetByEmail(r.Context(), req.Email)
	if err != nil {
		log.Println("Erreur GetByEmail:", err)
		http.Error(w, "internal error", http.StatusInternalServerError)
		return
	}
	if user == nil {
		log.Println("Utilisateur non trouvé pour:", req.Email)
	...
	if user.PasswordHash != req.Password {
		log.Printf("Mot de passe incorrect. Reçu: '%s', Attendu: '%s'", req.Password, user.PasswordHash)
		http.Error(w, "invalid credentials", http.StatusUnauthorized)
		return
	}
