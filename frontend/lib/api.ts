...
export const api = {
  ...
  students: {
    ...
    update: (id: string, data: any) => fetchAPI(`/api/students/${id}`, { method: "PUT", body: JSON.stringify(data) }),
  ...
  sections: {
    ...
    update: (id: string, data: any) => fetchAPI(`/api/sections/${id}`, { method: "PUT", body: JSON.stringify(data) }),
  ...
  invoices: {
    ...
    generate: (academicYearId?: string) =>
      fetchAPI("/api/invoices/generate", { method: "POST", body: JSON.stringify({ academic_year_id: academicYearId }) }),
